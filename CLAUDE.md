...
- **Migration de índice único nunca derruba o app (decisão de 2026-07-13).** `db.Database.Migrate()` não tem — e não deve ganhar — lógica de índice que possa falhar por dado legado: a migration `AddCpfUniqueIndex` teve o `CREATE UNIQUE INDEX` removido do `Up()` (virou no-op); a criação real do índice é um passo idempotente pós-migration em `Program.cs` (`IF NOT EXISTS`, dentro de try/catch, tentado em todo start). Motivo: produção com CPFs duplicados de antes de 2026-07-05 travava a migration inteira e o app não subia ("sistema não abre" / erro em várias telas). Falha na criação do índice é logada e populada em `AvisoSistemaService` (singleton, `CpfsDuplicados`), exibida pelo `BackupBanner`; o índice se cria sozinho no restart seguinte à correção em `/pessoas`. Ver `TIKAUM_SPEC.md` §9.
- **Log em arquivo (decisão de 2026-07-13).** `ArquivoLoggerProvider` registrado em `builder.Logging` grava em `data/logs/tikaum_AAAA-MM-DD.log` (retenção de 30 dias) — a máquina do estúdio roda por Tarefa Agendada sem console visível; sem isso, falhas fatais de startup eram invisíveis. Complementa os providers padrão, não substitui.
...
- **Build e deploy na prática: `git pull` + build direto na máquina do estúdio (decisão de 2026-07-13, corrige a suposição original de 2026-07-11).** A decisão original presumia uma máquina Linux do dev separada fazendo cross-publish e entregando o pacote pronto (por pen drive) para uma máquina do estúdio que "nunca compila". Na prática o usuário só tem duas máquinas — a de desenvolvimento (aqui) e a do estúdio — e prefere `git pull` direto no repo clonado na máquina do estúdio, seguido de `build_release.bat` **rodado ali mesmo** (ele já verifica/instala o SDK se faltar, lógica movida para lá desde a Rodada 39) e depois `install.bat`/`update.bat` **local**, sem pen drive. `build_release.sh` (Linux, cross-publish win-x64) continua existindo e funcionando para quem quiser gerar o pacote longe do estúdio, mas não é o caminho assumido por padrão — não descrever instruções que dependem de transferência física de pacote sem confirmar que é assim que o usuário está operando.
